...
const translations = {
  "zh-Hans": {
    meta: {
      title: "FindText - macOS 本地文件搜索与安全替换",
      description: "FindText 是一款本地优先的 macOS 应用，可通过问答模式、正则、历史记录、导出和安全批量替换预览来搜索代码、日志、配置和文档。",
      ogTitle: "FindText - macOS 本地文件搜索与安全替换",
    ...
    header: {
      aria: "站点导航",
      brandAria: "FindText 首页",
      navAria: "主要导航",
      download: "下载",
      nav: { guided: "问答模式", features: "功能", screenshots: "截图", privacy: "隐私", pricing: "价格" }
    },
    hero: {
      figureAria: "FindText 搜索界面",
      imageAlt: "FindText 问答搜索正在询问本地文件搜索条件",
      eyebrow: "macOS 本地优先搜索",
      title: "在每个本地文件里找到真正重要的内容。",
      lead: "FindText 把日常文件搜索变成清爽的 Mac 界面：不会正则时用问答模式，处理笔记、Markdown、报告、配置、日志和代码时也能切换到精确控制。",
      actionsAria: "主要操作",
    ...
    guidedIntro: {
      ...
      text: "流程会让搜索模型始终可见：每个回答都会变成搜索约束，最终报告会在 FindText 扫描本地文件夹前展示完整规则。",
      tabsAria: "问答模式步骤"
    },
    guided: {
      goal: { label: "目标", step: "第 1/6 步", title: "选择搜索应包含还是排除匹配行。", text: "第一个决定会转换为正向或反向搜索。用户可以要求查找符合描述特征的行，也可以查找不符合这些特征的行，而不用记 grep 参数或正则语法。", alt: "FindText 问答模式搜索目标步骤" },
      traits: { label: "特征", step: "第 2/6 步", title: "描述每一条匹配行必须包含什么。", text: "添加一个或多个词、短语、标识符或记得的片段。然后决定是所有特征都必须出现、任意一个即可，还是需要按顺序出现。", alt: "FindText 问答模式文本特征步骤" },
      boundaries: { label: "边界", step: "第 3/6 步", title: "限定文本在行内出现的位置。", text: "可选的“开头是”和“结尾是”问题能把模糊线索变成更精确的按行搜索。代码、日志和 Markdown 列表仍可允许前导缩进。", alt: "FindText 问答模式行首行尾步骤" },
      structures: { label: "结构", step: "第 4/6 步", title: "不用手写模式，也能加入常见结构。", text: "复选项覆盖 IP 地址、邮箱、日期、时间、十六进制数字、TODO 标记、错误词、英文字母、中文文本、端口和设备节点等可复用文本形态。", alt: "FindText 问答模式常见结构步骤" },
      options: { label: "选项", step: "第 5/6 步", title: "设置范围、大小写、文件类型和输出格式。", text: "最后的选项会把生成的规则连接到实际搜索任务：精确大小写、跳过目录、包含扩展名，以及结果显示匹配行、仅匹配内容还是文件路径。", alt: "FindText 问答模式搜索选项步骤" },
      report: { label: "报告", step: "第 6/6 步", title: "运行搜索前查看组装好的规则。", text: "报告让生成条件可审计。用户可以在开始本地扫描前检查目标、必需特征、大小写模式、跳过目录、文件类型和结果格式。", alt: "FindText 问答模式文本特征报告步骤" }
    },
    features: {
      eyebrow: "为重复工作而建",
      title: "简单到适合文档，精确到足够处理代码。",
      text: "FindText 让普通 Mac 用户可以通过问题和普通文本搜索，也为开发者、写作者、QA 团队和支持工程师提供更深入的控制项。",
      cards: [
    ...
    operator: {
      ...
      text: "FindText 适合简单的“我在哪里写过这个？”查询，也适合需要证据的深入检查。当一次快速查找变成文档清理、变更清单、发布检查、支持调查或代码审查时，它尤其有用。",
      aria: "FindText 优势",
      metrics: [
    ...
    gallery: {
      aria: "截图图库",
      plain: { label: "普通搜索", title: "用于直接查找的普通文本搜索。", text: "输入一个词、短语、标识符或错误信息，然后查看带高亮和文件上下文的分组匹配行。", alt: "FindText 普通文本搜索结果与高亮匹配" },
      regex: { label: "正则预设", title: "适合重复模式的正则预设。", text: "使用正则表达式和内置预设查找诊断信息、邮箱、IP、日期、TODO 标记和其他结构化文本。", alt: "FindText 使用诊断预设进行正则搜索" },
      export: { label: "导出结果", title: "将搜索结果导出为 Markdown、CSV、JSON 或纯文本。", text: "从结果工具栏打开导出菜单，选择适合分享、归档或继续处理的格式。", alt: "FindText 导出菜单显示 Markdown、CSV、JSON 和纯文本格式" },
      replace: { label: "替换预览", title: "任何写入前都先预览替换。", text: "按文件和出现位置检查每个候选项，只选择目标修改，并让 FindText 在应用前验证原始文本。", alt: "FindText 带有可选文件和匹配项的替换预览" },
      history: { label: "历史记录", title: "为完成的调查保留历史快照。", text: "回到之前的查询时，原始选项、结果数量、文件路径和高亮匹配仍然保留。", alt: "FindText 搜索历史显示此前的本地搜索结果" }
    },
    privacy: {
      eyebrow: "隐私与安全",
      title: "你的文件在你的 Mac 上处理。",
      text: "FindText 采用本地优先设计。当前应用行为不会收集数据、跟踪用户、创建云端索引或上传文件内容。文件夹访问通过 macOS 沙盒授权，并以安全作用域书签保存。",
      cards: [
    ...
    faq: {
      ...
      title: "安装前的几个细节。",
      items: [
        { question: "FindText 会上传我的文件吗？", answer: "不会。搜索在你的 Mac 本地运行。当前页面描述的功能不会把查询、文件路径或文件内容发送到服务器。" },
        ...
        { question: "可以只搜索某些文件类型吗？", answer: "可以。添加包含扩展名，例如 .md、.txt、.log、.swift 或 .docx。如果列表为空，FindText 会搜索未被规则跳过的可读文本文件。" }
      ]
    },
  ...
  "zh-Hant": {
    meta: {
      title: "FindText - macOS 本機檔案搜尋與安全取代",
      description: "FindText 是一款本機優先的 macOS App，可透過問答模式、正則、歷史記錄、匯出與安全批次取代預覽來搜尋程式碼、日誌、設定與文件。",
      ogTitle: "FindText - macOS 本機檔案搜尋與安全取代",
    ...
    header: {
      aria: "網站導覽",
      brandAria: "FindText 首頁",
      navAria: "主要導覽",
      download: "下載",
      nav: { guided: "問答模式", features: "功能", screenshots: "截圖", privacy: "隱私", pricing: "價格" }
    },
    hero: {
      figureAria: "FindText 搜尋介面",
      imageAlt: "FindText 問答搜尋正在詢問本機檔案搜尋條件",
      eyebrow: "macOS 本機優先搜尋",
      title: "在每個本機檔案裡找到真正重要的內容。",
      lead: "FindText 將日常檔案搜尋變成清爽的 Mac 介面：不會正則時用問答模式，處理筆記、Markdown、報告、設定、日誌與程式碼時也能切換到精確控制。",
      actionsAria: "主要操作",
    ...
    guidedIntro: {
      ...
      text: "流程會讓搜尋模型始終可見：每個回答都會變成搜尋限制，最終報告會在 FindText 掃描本機資料夾前顯示完整規則。",
      tabsAria: "問答模式步驟"
    },
    guided: {
      goal: { label: "目標", step: "第 1/6 步", title: "選擇搜尋應包含還是排除符合行。", text: "第一個決定會轉為正向或反向搜尋。使用者可以要求查找符合描述特徵的行，也可以查找不符合這些特徵的行，而不用記 grep 參數或正則語法。", alt: "FindText 問答模式搜尋目標步驟" },
      traits: { label: "特徵", step: "第 2/6 步", title: "描述每一條符合行必須包含什麼。", text: "加入一個或多個詞、短語、識別符或記得的片段。然後決定是所有特徵都必須出現、任意一個即可，還是需要依序出現。", alt: "FindText 問答模式文字特徵步驟" },
      boundaries: { label: "邊界", step: "第 3/6 步", title: "限定文字在行內出現的位置。", text: "可選的「開頭是」與「結尾是」問題能把模糊線索變成更精確的逐行搜尋。程式碼、日誌與 Markdown 清單仍可允許前導縮排。", alt: "FindText 問答模式行首行尾步驟" },
      structures: { label: "結構", step: "第 4/6 步", title: "不用手寫模式，也能加入常見結構。", text: "核取方塊涵蓋 IP 位址、電子郵件、日期、時間、十六進位數字、TODO 標記、錯誤詞、英文字母、中文文字、連接埠與裝置節點等可重複使用的文字形態。", alt: "FindText 問答模式常見結構步驟" },
      options: { label: "選項", step: "第 5/6 步", title: "設定範圍、大小寫、檔案類型與輸出格式。", text: "最後的選項會把產生的規則連接到實際搜尋工作：精確大小寫、跳過資料夾、包含副檔名，以及結果顯示符合行、僅符合內容還是檔案路徑。", alt: "FindText 問答模式搜尋選項步驟" },
      report: { label: "報告", step: "第 6/6 步", title: "執行搜尋前查看組裝好的規則。", text: "報告讓產生條件可稽核。使用者可以在開始本機掃描前檢查目標、必需特徵、大小寫模式、跳過資料夾、檔案類型與結果格式。", alt: "FindText 問答模式文字特徵報告步驟" }
    },
    features: {
      eyebrow: "為重複工作而建",
      title: "簡單到適合文件，精確到足夠處理程式碼。",
      text: "FindText 讓一般 Mac 使用者可以透過問題與普通文字搜尋，也為開發者、寫作者、QA 團隊與支援工程師提供更深入的控制項。",
      cards: [
    ...
    operator: {
      ...
      text: "FindText 適合簡單的「我在哪裡寫過這個？」查詢，也適合需要證據的深入檢查。當一次快速查找變成文件清理、變更清單、發布檢查、支援調查或程式碼審查時，它尤其有用。",
      aria: "FindText 優勢",
      metrics: [
    ...
    gallery: {
      aria: "截圖圖庫",
      plain: { label: "普通搜尋", title: "用於直接查找的普通文字搜尋。", text: "輸入一個詞、短語、識別符或錯誤訊息，然後查看帶高亮與檔案上下文的分組符合行。", alt: "FindText 普通文字搜尋結果與高亮符合項" },
      regex: { label: "正則預設", title: "適合重複模式的正則預設。", text: "使用正則表示式與內建預設查找診斷資訊、電子郵件、IP、日期、TODO 標記與其他結構化文字。", alt: "FindText 使用診斷預設進行正則搜尋" },
      export: { label: "匯出結果", title: "將搜尋結果匯出為 Markdown、CSV、JSON 或純文字。", text: "從結果工具列開啟匯出選單，選擇最適合分享、封存或後續處理的格式。", alt: "FindText 匯出選單顯示 Markdown、CSV、JSON 與純文字格式" },
      replace: { label: "取代預覽", title: "任何寫入前都先預覽取代。", text: "依檔案與出現位置檢查每個候選項，只選擇目標修改，並讓 FindText 在套用前驗證原始文字。", alt: "FindText 帶有可選檔案與符合項的取代預覽" },
      history: { label: "歷史記錄", title: "為完成的調查保留歷史快照。", text: "回到之前的查詢時，原始選項、結果數量、檔案路徑與高亮符合項仍然保留。", alt: "FindText 搜尋歷史顯示此前的本機搜尋結果" }
    },
    privacy: {
      eyebrow: "隱私與安全",
      title: "你的檔案在你的 Mac 上處理。",
      text: "FindText 採用本機優先設計。當前 App 行為不會收集資料、追蹤使用者、建立雲端索引或上傳檔案內容。資料夾存取透過 macOS 沙盒授權，並以安全作用域書籤保存。",
      cards: [
    ...
    faq: {
      ...
      title: "安裝前的幾個細節。",
      items: [
        { question: "FindText 會上傳我的檔案嗎？", answer: "不會。搜尋在你的 Mac 本機執行。當前頁面描述的功能不會把查詢、檔案路徑或檔案內容傳送到伺服器。" },
        ...
        { question: "可以只搜尋某些檔案類型嗎？", answer: "可以。加入包含副檔名，例如 .md、.txt、.log、.swift 或 .docx。如果列表為空，FindText 會搜尋未被規則跳過的可讀文字檔。" }
      ]
    },
  ...
  en: {
    meta: {
      title: "FindText - Local file search and safe replacement for macOS",
      description: "FindText is a local-first macOS app for searching code, logs, configs, and documents with Question Mode, regex, history, export, and safe batch replacement previews.",
      ogTitle: "FindText - Local file search and safe replacement for macOS",
    ...
    header: {
      ...
      brandAria: "FindText home",
      navAria: "Primary navigation",
      download: "Download",
      nav: { guided: "Question Mode", features: "Features", screenshots: "Screenshots", privacy: "Privacy", pricing: "Pricing" }
    },
    hero: {
      figureAria: "FindText search interface",
      imageAlt: "FindText Guided Search asking questions for a local file search",
      eyebrow: "Local-first search for macOS",
      title: "Find what matters across every local file.",
      lead: "FindText gives everyday file search a calm Mac interface: use Question Mode when you do not know regex, or switch to precise controls for notes, Markdown, reports, configs, logs, and code.",
    ...
    guidedIntro: {
      ...
      text: "The flow keeps the mental model visible: every answer becomes a search constraint, and the final report shows the assembled rule before FindText scans your local folders.",
      tabsAria: "Question Mode steps"
    },
    guided: {
      goal: { label: "Goal", step: "Step 1 of 6", title: "Choose whether the search should include or exclude matching lines.", text: "The first decision maps to positive or inverse search. Users can ask for lines matching described traits, or lines that do not match those traits, without needing grep flags or regex syntax.", alt: "FindText Question Mode search goal step" },
      traits: { label: "Traits", step: "Step 2 of 6", title: "Describe what each matching line must contain.", text: "Add one or more words, phrases, identifiers, or remembered fragments. Then decide whether all traits are required, any one trait is enough, or the traits must appear in order.", alt: "FindText Question Mode text traits step" },
      boundaries: { label: "Boundaries", step: "Step 3 of 6", title: "Limit where text appears inside a line.", text: "Optional starts-with and ends-with questions help turn a vague text clue into a tighter line-level search. Leading indentation can stay allowed for code, logs, and Markdown lists.", alt: "FindText Question Mode line starts and ends step" },
      structures: { label: "Structures", step: "Step 4 of 6", title: "Add common structures without writing patterns by hand.", text: "Checkboxes cover reusable text shapes such as IP addresses, emails, dates, times, hex numbers, TODO markers, error terms, English letters, Chinese text, ports, and device nodes.", alt: "FindText Question Mode common structures step" },
      options: { label: "Options", step: "Step 5 of 6", title: "Set scope, case behavior, file types, and output format.", text: "The final options connect the generated rule to the practical search job: exact case, skipped folders, included extensions, and whether results show matching lines, matches only, or file paths.", alt: "FindText Question Mode search options step" },
      report: { label: "Report", step: "Step 6 of 6", title: "Review the assembled rule before running the search.", text: "The report makes the generated criteria auditable. Users can check the goal, required traits, case mode, skipped folders, file types, and result format before starting the local scan.", alt: "FindText Question Mode text trait report step" }
    },
    features: {
      eyebrow: "Built for repeated work",
      title: "Simple enough for documents. Precise enough for code.",
      text: "FindText helps ordinary Mac users search through questions and plain text, while giving developers, writers, QA teams, and support engineers the deeper controls they expect.",
      cards: [
    ...
    operator: {
      ...
      text: "FindText works for simple \"where did I write that?\" searches and for deeper inspections that need proof. It is especially useful when a quick lookup turns into a document cleanup, change list, release check, support investigation, or code review.",
      aria: "FindText strengths",
      metrics: [
    ...
    gallery: {
      ...
      plain: { label: "Plain Search", title: "Plain text search for direct lookups.", text: "Type a word, phrase, identifier, or error message, then review grouped matching lines with highlights and file context.", alt: "FindText plain text search results with highlighted matches" },
      regex: { label: "Regex Preset", title: "Regex presets for patterns that repeat.", text: "Use regular expressions and built-in presets for diagnostics, email addresses, IPs, dates, TODO markers, and other structured text.", alt: "FindText regular expression search using a diagnostics preset" },
      export: { label: "Export Results", title: "Export search results as Markdown, CSV, JSON, or plain text.", text: "Open the export menu from the results toolbar and choose the format that best fits sharing, archiving, or further processing.", alt: "FindText export menu showing Markdown, CSV, JSON, and plain text formats" },
      replace: { label: "Replace Preview", title: "Replacement previews before any write.", text: "Review each candidate by file and occurrence, select only the intended changes, and let FindText verify the source text before applying.", alt: "FindText replacement preview with selectable file and occurrence changes" },
      history: { label: "History", title: "History snapshots for finished investigations.", text: "Return to a previous query with its original options, result counts, file paths, and highlighted matches still intact.", alt: "FindText search history showing previous local search results" }
    },
    privacy: {
      eyebrow: "Privacy and safety",
      title: "Your files are processed on your Mac.",
      text: "FindText is local-first by design. The current app behavior does not collect data, track users, create cloud indexes, or upload file content. Folder access is granted through the macOS sandbox and stored as security-scoped bookmarks.",
      cards: [
    ...
    faq: {
      ...
      items: [
        { question: "Does FindText upload my files?", answer: "No. Search runs locally on your Mac. Queries, file paths, and file content are not sent to a server by the current app features described here." },
        ...
        { question: "Can I limit a search to certain file types?", answer: "Yes. Add included extensions such as .md, .txt, .log, .swift, or .docx. If the list is empty, FindText searches readable text files that are not skipped by your rules." }
      ]
    },
  ...
  de: {
    meta: {
      title: "FindText - Lokale Dateisuche und sicheres Ersetzen für macOS",
      description: "FindText ist eine lokal arbeitende macOS-App zum Durchsuchen von Code, Logs, Konfigurationen und Dokumenten mit Fragemodus, Regex, Verlauf, Export und sicheren Vorschauen für Stapelersetzungen.",
      ogTitle: "FindText - Lokale Dateisuche und sicheres Ersetzen für macOS",
    ...
    header: {
      ...
      brandAria: "FindText Startseite",
      navAria: "Hauptnavigation",
      download: "Download",
      nav: { guided: "Fragemodus", features: "Funktionen", screenshots: "Screenshots", privacy: "Datenschutz", pricing: "Preise" }
    },
    hero: {
      figureAria: "FindText Suchoberfläche",
      imageAlt: "FindText Guided Search stellt Fragen für eine lokale Dateisuche",
      eyebrow: "Lokale Suche zuerst für macOS",
      title: "Finde, was in deinen lokalen Dateien zählt.",
      lead: "FindText gibt der alltäglichen Dateisuche eine ruhige Mac-Oberfläche: Nutze den Fragemodus, wenn du kein Regex kennst, oder wechsle zu präzisen Optionen für Notizen, Markdown, Berichte, Konfigurationen, Logs und Code.",
    ...
    operator: {
      ...
      text: "FindText eignet sich für einfache Suchen wie „Wo habe ich das geschrieben?“ und für tiefere Prüfungen, die Belege brauchen. Besonders hilfreich ist es, wenn aus einem kurzen Nachschlagen eine Dokumentbereinigung, Änderungsliste, Release-Prüfung, Supportanalyse oder Code-Review wird.",
      aria: "Stärken von FindText",
      metrics: [
    ...
    gallery: {
      ...
      plain: { label: "Klartextsuche", title: "Klartextsuche für direkte Nachschläge.", text: "Gib ein Wort, eine Phrase, einen Bezeichner oder eine Fehlermeldung ein und prüfe gruppierte Trefferzeilen mit Hervorhebungen und Dateikontext.", alt: "FindText Klartextsuchergebnisse mit hervorgehobenen Treffern" },
      regex: { label: "Regex-Voreinstellung", title: "Regex-Voreinstellungen für wiederkehrende Muster.", text: "Nutze reguläre Ausdrücke und integrierte Voreinstellungen für Diagnosen, E-Mail-Adressen, IPs, Daten, TODO-Marker und andere strukturierte Texte.", alt: "FindText Regex-Suche mit Diagnose-Voreinstellung" },
      export: { label: "Ergebnisse exportieren", title: "Exportiere Suchergebnisse als Markdown, CSV, JSON oder Klartext.", text: "Öffne das Exportmenü in der Ergebnisleiste und wähle das Format, das am besten zum Teilen, Archivieren oder Weiterverarbeiten passt.", alt: "FindText Exportmenü mit Markdown-, CSV-, JSON- und Klartextformaten" },
      replace: { label: "Ersetzungsvorschau", title: "Ersetzungsvorschauen vor jedem Schreiben.", text: "Prüfe jeden Kandidaten nach Datei und Vorkommen, wähle nur beabsichtigte Änderungen und lass FindText den Ausgangstext vor dem Anwenden prüfen.", alt: "FindText Ersetzungsvorschau mit auswählbaren Dateien und Vorkommen" },
      history: { label: "Verlauf", title: "Verlaufsschnappschüsse für abgeschlossene Untersuchungen.", text: "Kehre zu einer früheren Abfrage zurück, bei der ursprüngliche Optionen, Ergebniszahlen, Dateipfade und Hervorhebungen erhalten bleiben.", alt: "FindText Suchverlauf mit früheren lokalen Suchergebnissen" }
    },
    privacy: {
      eyebrow: "Datenschutz und Sicherheit",
      title: "Deine Dateien werden auf deinem Mac verarbeitet.",
      text: "FindText ist lokal ausgerichtet. Die aktuellen App-Funktionen sammeln keine Daten, verfolgen keine Nutzer, erstellen keine Cloud-Indizes und laden keine Dateiinhalte hoch. Ordnerzugriff erfolgt über die macOS-Sandbox und wird als sicherheitsbezogenes Lesezeichen gespeichert.",
      cards: [
    ...
    faq: {
      ...
      items: [
        { question: "Lädt FindText meine Dateien hoch?", answer: "Nein. Die Suche läuft lokal auf deinem Mac. Abfragen, Dateipfade und Dateiinhalte werden von den hier beschriebenen Funktionen nicht an einen Server gesendet." },
        ...
        { question: "Kann ich die Suche auf bestimmte Dateitypen beschränken?", answer: "Ja. Füge Erweiterungen wie .md, .txt, .log, .swift oder .docx hinzu. Ist die Liste leer, durchsucht FindText lesbare Textdateien, die nicht durch deine Regeln übersprungen werden." }
      ]
    },
  ...
  es: {
    meta: {
      title: "FindText - Búsqueda local de archivos y reemplazo seguro para macOS",
      description: "FindText es una app de macOS local-first para buscar en código, registros, configuraciones y documentos con Modo Pregunta, regex, historial, exportación y vistas previas seguras de reemplazo por lotes.",
      ogTitle: "FindText - Búsqueda local de archivos y reemplazo seguro para macOS",
    ...
    header: {
      ...
      brandAria: "Inicio de FindText",
      navAria: "Navegación principal",
      download: "Descargar",
      nav: { guided: "Modo Pregunta", features: "Funciones", screenshots: "Capturas", privacy: "Privacidad", pricing: "Precio" }
    },
    hero: {
      figureAria: "Interfaz de búsqueda de FindText",
      imageAlt: "FindText Guided Search haciendo preguntas para una búsqueda local de archivos",
      eyebrow: "Búsqueda local-first para macOS",
      title: "Encuentra lo importante en cada archivo local.",
      lead: "FindText convierte la búsqueda diaria de archivos en una interfaz Mac tranquila: usa el Modo Pregunta si no sabes regex, o cambia a controles precisos para notas, Markdown, informes, configuraciones, registros y código.",
    ...
    features: {
      ...
      text: "FindText ayuda a usuarios cotidianos de Mac a buscar con preguntas y texto simple, mientras da a desarrolladores, redactores, QA y soporte los controles profundos que esperan.",
      cards: [
    ...
    operator: {
      ...
      text: "FindText sirve para búsquedas simples como “¿dónde escribí eso?” y para inspecciones profundas que necesitan prueba. Es especialmente útil cuando una consulta rápida se vuelve limpieza de documentos, lista de cambios, revisión de release, investigación de soporte o revisión de código.",
      aria: "Fortalezas de FindText",
      metrics: [
    ...
    gallery: {
      ...
      plain: { label: "Texto simple", title: "Búsqueda de texto simple para consultas directas.", text: "Escribe una palabra, frase, identificador o mensaje de error y revisa líneas agrupadas con resaltados y contexto de archivo.", alt: "Resultados de búsqueda de texto simple en FindText con coincidencias resaltadas" },
      regex: { label: "Preset regex", title: "Presets regex para patrones que se repiten.", text: "Usa expresiones regulares y presets integrados para diagnósticos, correos, IPs, fechas, marcadores TODO y otros textos estructurados.", alt: "Búsqueda regex de FindText usando un preset de diagnóstico" },
      export: { label: "Exportar resultados", title: "Exporta resultados como Markdown, CSV, JSON o texto plano.", text: "Abre el menú de exportación desde la barra de resultados y elige el formato ideal para compartir, archivar o procesar después.", alt: "Menú de exportación de FindText con formatos Markdown, CSV, JSON y texto plano" },
      replace: { label: "Vista previa", title: "Vistas previas de reemplazo antes de escribir.", text: "Revisa cada candidato por archivo y aparición, selecciona solo los cambios previstos y deja que FindText verifique el texto original antes de aplicar.", alt: "Vista previa de reemplazo de FindText con cambios seleccionables por archivo y aparición" },
      history: { label: "Historial", title: "Instantáneas de historial para investigaciones terminadas.", text: "Vuelve a una consulta anterior con sus opciones originales, recuentos, rutas y coincidencias resaltadas intactas.", alt: "Historial de búsqueda de FindText con resultados locales anteriores" }
    },
    privacy: {
      eyebrow: "Privacidad y seguridad",
      title: "Tus archivos se procesan en tu Mac.",
      text: "FindText está diseñado local-first. El comportamiento actual de la app no recopila datos, no rastrea usuarios, no crea índices en la nube ni sube contenido de archivos. El acceso a carpetas se concede con el sandbox de macOS y se guarda como marcadores con alcance de seguridad.",
      cards: [
    ...
    faq: {
      ...
      items: [
        { question: "¿FindText sube mis archivos?", answer: "No. La búsqueda se ejecuta localmente en tu Mac. Las consultas, rutas y contenido de archivos no se envían a un servidor con las funciones descritas aquí." },
        ...
        { question: "¿Puedo limitar la búsqueda a ciertos tipos de archivo?", answer: "Sí. Añade extensiones como .md, .txt, .log, .swift o .docx. Si la lista está vacía, FindText busca archivos de texto legibles que tus reglas no omitan." }
      ]
    },
  ...
  fr: {
    meta: {
      title: "FindText - Recherche locale de fichiers et remplacement sûr pour macOS",
      description: "FindText est une app macOS locale d’abord pour rechercher dans le code, les journaux, les configurations et les documents avec le mode Questions, regex, historique, export et aperçus sûrs de remplacement en lot.",
      ogTitle: "FindText - Recherche locale de fichiers et remplacement sûr pour macOS",
    ...
    header: {
      ...
      brandAria: "Accueil FindText",
      navAria: "Navigation principale",
      download: "Télécharger",
      nav: { guided: "Mode Questions", features: "Fonctions", screenshots: "Captures", privacy: "Confidentialité", pricing: "Prix" }
    },
    hero: {
      figureAria: "Interface de recherche FindText",
      imageAlt: "FindText Guided Search pose des questions pour une recherche locale de fichiers",
      eyebrow: "Recherche locale d’abord pour macOS",
      title: "Trouvez l’essentiel dans chaque fichier local.",
      lead: "FindText donne à la recherche de fichiers quotidienne une interface Mac calme : utilisez le mode Questions si vous ne connaissez pas les regex, ou passez à des contrôles précis pour notes, Markdown, rapports, configs, journaux et code.",
    ...
    guidedIntro: {
      ...
      text: "Le flux garde le modèle mental visible : chaque réponse devient une contrainte de recherche, et le rapport final montre la règle assemblée avant que FindText analyse vos dossiers locaux.",
    ...
    features: {
      ...
      text: "FindText aide les utilisateurs Mac ordinaires à chercher avec des questions et du texte simple, tout en donnant aux développeurs, rédacteurs, équipes QA et support les contrôles plus profonds attendus.",
      cards: [
    ...
    gallery: {
      ...
      plain: { label: "Texte simple", title: "Recherche de texte simple pour les consultations directes.", text: "Saisissez un mot, une phrase, un identifiant ou un message d’erreur, puis examinez les lignes groupées avec surlignages et contexte de fichier.", alt: "Résultats de recherche texte simple de FindText avec correspondances surlignées" },
      regex: { label: "Préréglage regex", title: "Préréglages regex pour les motifs répétés.", text: "Utilisez expressions régulières et préréglages intégrés pour diagnostics, e-mails, IP, dates, marqueurs TODO et autres textes structurés.", alt: "Recherche regex FindText avec préréglage de diagnostic" },
      export: { label: "Exporter les résultats", title: "Exportez les résultats en Markdown, CSV, JSON ou texte brut.", text: "Ouvrez le menu d’export dans la barre des résultats et choisissez le format adapté au partage, à l’archivage ou au traitement ultérieur.", alt: "Menu d’export FindText affichant les formats Markdown, CSV, JSON et texte brut" },
      replace: { label: "Aperçu remplacement", title: "Aperçus de remplacement avant toute écriture.", text: "Examinez chaque candidat par fichier et occurrence, sélectionnez seulement les changements voulus et laissez FindText vérifier le texte source avant application.", alt: "Aperçu de remplacement FindText avec changements sélectionnables par fichier et occurrence" },
      history: { label: "Historique", title: "Instantanés d’historique pour les enquêtes terminées.", text: "Revenez à une requête précédente avec options originales, nombres de résultats, chemins et correspondances surlignées toujours intacts.", alt: "Historique de recherche FindText montrant des résultats locaux précédents" }
    },
    privacy: {
    ...
    faq: {
      ...
      items: [
        { question: "FindText téléverse-t-il mes fichiers ?", answer: "Non. La recherche s’exécute localement sur votre Mac. Les requêtes, chemins et contenus de fichiers ne sont pas envoyés à un serveur par les fonctions décrites ici." },
        ...
        { question: "Puis-je limiter une recherche à certains types de fichiers ?", answer: "Oui. Ajoutez des extensions comme .md, .txt, .log, .swift ou .docx. Si la liste est vide, FindText recherche les fichiers texte lisibles qui ne sont pas ignorés par vos règles." }
      ]
    },
  ...
  it: {
    meta: {
      title: "FindText - Ricerca locale nei file e sostituzione sicura per macOS",
      description: "FindText è un’app macOS local-first per cercare in codice, log, configurazioni e documenti con Modalità Domande, regex, cronologia, esportazione e anteprime sicure di sostituzione in batch.",
      ogTitle: "FindText - Ricerca locale nei file e sostituzione sicura per macOS",
    ...
    hero: {
      figureAria: "Interfaccia di ricerca FindText",
      imageAlt: "FindText Guided Search pone domande per una ricerca locale nei file",
      eyebrow: "Ricerca local-first per macOS",
      title: "Trova ciò che conta in ogni file locale.",
      lead: "FindText trasforma la ricerca quotidiana nei file in un’interfaccia Mac ordinata: usa la Modalità Domande se non conosci le regex, oppure passa a controlli precisi per note, Markdown, report, configurazioni, log e codice.",
    ...
    guidedIntro: {
      ...
      text: "Il flusso mantiene visibile il modello mentale: ogni risposta diventa un vincolo di ricerca e il report finale mostra la regola assemblata prima che FindText analizzi le cartelle locali.",
      tabsAria: "Passaggi della Modalità Domande"
    },
    guided: {
      goal: { label: "Obiettivo", step: "Passaggio 1 di 6", title: "Scegli se la ricerca deve includere o escludere le righe corrispondenti.", text: "La prima decisione diventa una ricerca positiva o inversa. Puoi chiedere righe che corrispondono ai tratti descritti, o righe che non corrispondono, senza flag grep o sintassi regex.", alt: "Passaggio obiettivo di ricerca nella Modalità Domande di FindText" },
      traits: { label: "Tratti", step: "Passaggio 2 di 6", title: "Descrivi cosa deve contenere ogni riga corrispondente.", text: "Aggiungi una o più parole, frasi, identificatori o frammenti ricordati. Poi decidi se tutti i tratti sono richiesti, se ne basta uno o se devono apparire in ordine.", alt: "Passaggio tratti di testo nella Modalità Domande di FindText" },
      boundaries: { label: "Confini", step: "Passaggio 3 di 6", title: "Limita dove appare il testo dentro una riga.", text: "Le domande opzionali su inizio e fine riga aiutano a trasformare un indizio vago in una ricerca per riga più stretta. L’indentazione iniziale può restare permessa per codice, log e liste Markdown.", alt: "Passaggio inizio e fine riga nella Modalità Domande di FindText" },
      structures: { label: "Strutture", step: "Passaggio 4 di 6", title: "Aggiungi strutture comuni senza scrivere pattern a mano.", text: "Le caselle coprono forme riutilizzabili come indirizzi IP, email, date, orari, numeri esadecimali, TODO, termini di errore, lettere inglesi, testo cinese, porte e nodi dispositivo.", alt: "Passaggio strutture comuni nella Modalità Domande di FindText" },
      options: { label: "Opzioni", step: "Passaggio 5 di 6", title: "Imposta ambito, maiuscole/minuscole, tipi di file e output.", text: "Le opzioni finali collegano la regola generata al lavoro pratico: maiuscole esatte, cartelle saltate, estensioni incluse e risultati come righe, sole corrispondenze o percorsi file.", alt: "Passaggio opzioni di ricerca nella Modalità Domande di FindText" },
      report: { label: "Report", step: "Passaggio 6 di 6", title: "Rivedi la regola assemblata prima di cercare.", text: "Il report rende verificabili i criteri. Puoi controllare obiettivo, tratti richiesti, maiuscole/minuscole, cartelle saltate, tipi di file e formato prima della scansione locale.", alt: "Passaggio report dei tratti di testo nella Modalità Domande di FindText" }
    },
    features: {
      eyebrow: "Creato per il lavoro ripetuto",
      title: "Abbastanza semplice per i documenti. Abbastanza preciso per il codice.",
      text: "FindText aiuta gli utenti Mac di tutti i giorni a cercare con domande e testo semplice, offrendo a sviluppatori, writer, QA e supporto i controlli più profondi che si aspettano.",
      cards: [
    ...
    operator: {
      ...
      text: "FindText funziona per semplici ricerche “dove l’ho scritto?” e per controlli più profondi che richiedono prove. È particolarmente utile quando una ricerca rapida diventa pulizia documenti, lista modifiche, controllo release, indagine supporto o code review.",
      aria: "Punti di forza di FindText",
      metrics: [
    ...
    gallery: {
      ...
      plain: { label: "Testo semplice", title: "Ricerca di testo semplice per consultazioni dirette.", text: "Digita una parola, frase, identificatore o messaggio di errore, poi rivedi righe corrispondenti raggruppate con evidenziazioni e contesto file.", alt: "Risultati di ricerca testo semplice in FindText con corrispondenze evidenziate" },
      regex: { label: "Preset regex", title: "Preset regex per pattern ricorrenti.", text: "Usa espressioni regolari e preset integrati per diagnostica, email, IP, date, marker TODO e altri testi strutturati.", alt: "Ricerca regex di FindText con preset diagnostico" },
      export: { label: "Esporta risultati", title: "Esporta i risultati come Markdown, CSV, JSON o testo semplice.", text: "Apri il menu di esportazione dalla barra dei risultati e scegli il formato più adatto a condivisione, archivio o elaborazione successiva.", alt: "Menu di esportazione FindText con formati Markdown, CSV, JSON e testo semplice" },
      replace: { label: "Anteprima sostituzione", title: "Anteprime di sostituzione prima di ogni scrittura.", text: "Rivedi ogni candidato per file e occorrenza, seleziona solo le modifiche previste e lascia che FindText verifichi il testo sorgente prima di applicare.", alt: "Anteprima sostituzione di FindText con modifiche selezionabili per file e occorrenza" },
      history: { label: "Cronologia", title: "Snapshot della cronologia per indagini concluse.", text: "Torna a una query precedente con opzioni originali, conteggi, percorsi e corrispondenze evidenziate ancora intatti.", alt: "Cronologia di ricerca FindText con risultati locali precedenti" }
    },
    privacy: {
      eyebrow: "Privacy e sicurezza",
      title: "I tuoi file vengono elaborati sul tuo Mac.",
      text: "FindText è local-first per progettazione. Le funzioni attuali non raccolgono dati, non tracciano utenti, non creano indici cloud e non caricano contenuti dei file. L’accesso alle cartelle avviene tramite sandbox macOS e viene salvato come bookmark con ambito di sicurezza.",
      cards: [
    ...
    faq: {
      ...
      items: [
        { question: "FindText carica i miei file?", answer: "No. La ricerca viene eseguita localmente sul tuo Mac. Query, percorsi e contenuti dei file non vengono inviati a un server dalle funzioni descritte qui." },
        ...
        { question: "Posso limitare la ricerca ad alcuni tipi di file?", answer: "Sì. Aggiungi estensioni come .md, .txt, .log, .swift o .docx. Se l’elenco è vuoto, FindText cerca nei file di testo leggibili non esclusi dalle tue regole." }
      ]
    },
  ...
  ja: {
    meta: {
      title: "FindText - macOS のローカルファイル検索と安全な置換",
      description: "FindText は、質問モード、正規表現、履歴、書き出し、安全な一括置換プレビューでコード、ログ、設定、ドキュメントを検索できるローカル優先の macOS アプリです。",
      ogTitle: "FindText - macOS のローカルファイル検索と安全な置換",
    ...
    header: {
      aria: "サイトナビゲーション",
      brandAria: "FindText ホーム",
      navAria: "メインナビゲーション",
      download: "ダウンロード",
      nav: { guided: "質問モード", features: "機能", screenshots: "スクリーンショット", privacy: "プライバシー", pricing: "価格" }
    },
    hero: {
      figureAria: "FindText の検索インターフェイス",
      imageAlt: "FindText Guided Search がローカルファイル検索の条件を質問している画面",
      eyebrow: "macOS のローカル優先検索",
      title: "すべてのローカルファイルから大事な内容を見つける。",
      lead: "FindText は日常のファイル検索を落ち着いた Mac インターフェイスにします。正規表現がわからないときは質問モードを使い、ノート、Markdown、レポート、設定、ログ、コードには精密なコントロールへ切り替えられます。",
      actionsAria: "主な操作",
    ...
    guidedIntro: {
      ...
      text: "この流れでは検索モデルが常に見えます。各回答は検索条件になり、最終レポートは FindText がローカルフォルダをスキャンする前に組み立てたルールを示します。",
      tabsAria: "質問モードの手順"
    },
    guided: {
      goal: { label: "目的", step: "ステップ 1 / 6", title: "一致する行を含めるか、除外するかを選びます。", text: "最初の選択は通常検索または逆検索になります。grep のフラグや正規表現の構文を知らなくても、指定した特性に一致する行、または一致しない行を探せます。", alt: "FindText 質問モードの検索目的ステップ" },
      traits: { label: "特性", step: "ステップ 2 / 6", title: "各一致行に含まれるべき内容を説明します。", text: "単語、フレーズ、識別子、覚えている断片を 1 つ以上追加します。その後、すべて必要か、どれか 1 つでよいか、順序が必要かを決めます。", alt: "FindText 質問モードのテキスト特性ステップ" },
      boundaries: { label: "境界", step: "ステップ 3 / 6", title: "テキストが行のどこに現れるかを限定します。", text: "任意の行頭・行末の質問で、曖昧な手がかりをより狭い行単位の検索にできます。コード、ログ、Markdown リストでは先頭インデントを許可したままにできます。", alt: "FindText 質問モードの行頭と行末ステップ" },
      structures: { label: "構造", step: "ステップ 4 / 6", title: "手書きのパターンなしで一般的な構造を追加します。", text: "チェックボックスで IP アドレス、メール、日付、時刻、16 進数、TODO、エラー語、英字、中国語テキスト、ポート、デバイスノードなどの再利用可能な形を指定できます。", alt: "FindText 質問モードの一般構造ステップ" },
      options: { label: "オプション", step: "ステップ 5 / 6", title: "範囲、大文字小文字、ファイルタイプ、出力形式を設定します。", text: "最後のオプションで生成ルールを実際の検索に接続します。大文字小文字の完全一致、除外フォルダ、対象拡張子、結果を一致行・一致部分のみ・ファイルパスのどれで表示するかを選びます。", alt: "FindText 質問モードの検索オプションステップ" },
      report: { label: "レポート", step: "ステップ 6 / 6", title: "検索前に組み立てたルールを確認します。", text: "レポートにより生成条件を確認できます。ローカルスキャンを始める前に、目的、必須特性、大文字小文字、除外フォルダ、ファイルタイプ、結果形式をチェックできます。", alt: "FindText 質問モードのテキスト特性レポートステップ" }
    },
    features: {
      eyebrow: "繰り返す作業のために",
      title: "ドキュメントには簡単に。コードには十分に正確に。",
      text: "FindText は一般的な Mac ユーザーには質問とプレーンテキスト検索を、開発者、ライター、QA、サポート担当者には必要な深いコントロールを提供します。",
      cards: [
    ...
    operator: {
      ...
      text: "FindText は「どこに書いたっけ？」という簡単な検索にも、証拠が必要な深い調査にも使えます。短い検索が文書整理、変更リスト、リリース確認、サポート調査、コードレビューに変わるとき特に役立ちます。",
      aria: "FindText の強み",
      metrics: [
    ...
    gallery: {
      aria: "スクリーンショットギャラリー",
      plain: { label: "通常検索", title: "直接探すためのプレーンテキスト検索。", text: "単語、フレーズ、識別子、エラーメッセージを入力し、ハイライトとファイル文脈付きでグループ化された一致行を確認します。", alt: "FindText のプレーンテキスト検索結果とハイライトされた一致" },
      regex: { label: "正規表現プリセット", title: "繰り返すパターンのための正規表現プリセット。", text: "正規表現と内蔵プリセットで、診断情報、メールアドレス、IP、日付、TODO マーカー、その他の構造化テキストを検索します。", alt: "診断プリセットを使った FindText の正規表現検索" },
      export: { label: "結果を書き出し", title: "検索結果を Markdown、CSV、JSON、プレーンテキストで書き出し。", text: "結果ツールバーから書き出しメニューを開き、共有、保存、後処理に合う形式を選べます。", alt: "Markdown、CSV、JSON、プレーンテキスト形式を表示する FindText の書き出しメニュー" },
      replace: { label: "置換プレビュー", title: "書き込む前に必ず置換をプレビュー。", text: "各候補をファイルと出現箇所ごとに確認し、意図した変更だけを選択して、適用前に FindText が元テキストを検証します。", alt: "ファイルと出現箇所を選択できる FindText の置換プレビュー" },
      history: { label: "履歴", title: "完了した調査のための履歴スナップショット。", text: "以前の検索に戻っても、元のオプション、結果数、ファイルパス、ハイライトされた一致が保持されています。", alt: "以前のローカル検索結果を表示する FindText の検索履歴" }
    },
    privacy: {
      eyebrow: "プライバシーと安全性",
      title: "ファイルはあなたの Mac 上で処理されます。",
      text: "FindText はローカル優先で設計されています。現在のアプリ機能はデータ収集、ユーザー追跡、クラウドインデックス作成、ファイル内容のアップロードを行いません。フォルダアクセスは macOS サンドボックスで許可され、セキュリティスコープ付きブックマークとして保存されます。",
      cards: [
    ...
    faq: {
      ...
      items: [
        { question: "FindText はファイルをアップロードしますか？", answer: "いいえ。検索は Mac 上でローカルに実行されます。ここで説明している機能では、クエリ、ファイルパス、ファイル内容はサーバーへ送信されません。" },
        ...
        { question: "特定のファイルタイプだけを検索できますか？", answer: "はい。.md、.txt、.log、.swift、.docx などの拡張子を追加できます。リストが空の場合、FindText はルールで除外されていない読み取り可能なテキストファイルを検索します。" }
      ]
    },
  ...
  ru: {
    meta: {
      title: "FindText - Локальный поиск по файлам и безопасная замена для macOS",
      description: "FindText — локальное приложение macOS для поиска по коду, логам, конфигам и документам с режимом вопросов, regex, историей, экспортом и безопасными предпросмотрами пакетной замены.",
      ogTitle: "FindText - Локальный поиск по файлам и безопасная замена для macOS",
    ...
    header: {
      ...
      brandAria: "Главная FindText",
      navAria: "Основная навигация",
      download: "Скачать",
      nav: { guided: "Режим вопросов", features: "Функции", screenshots: "Скриншоты", privacy: "Приватность", pricing: "Цена" }
    },
    hero: {
      figureAria: "Интерфейс поиска FindText",
      imageAlt: "FindText Guided Search задает вопросы для локального поиска по файлам",
      eyebrow: "Локальный поиск для macOS",
      title: "Находите важное в каждом локальном файле.",
      lead: "FindText превращает повседневный поиск по файлам в спокойный Mac-интерфейс: используйте режим вопросов, если не знаете regex, или переходите к точным настройкам для заметок, Markdown, отчетов, конфигов, логов и кода.",
    ...
    operator: {
      ...
      text: "FindText подходит для простых поисков вроде «где я это писал?» и для более глубоких проверок, которым нужны доказательства. Особенно полезен, когда быстрый поиск превращается в чистку документов, список изменений, проверку релиза, расследование поддержки или code review.",
      aria: "Сильные стороны FindText",
      metrics: [
    ...
    gallery: {
      ...
      plain: { label: "Обычный поиск", title: "Поиск обычного текста для прямых запросов.", text: "Введите слово, фразу, идентификатор или сообщение об ошибке и просмотрите сгруппированные строки с подсветкой и контекстом файла.", alt: "Результаты поиска обычного текста FindText с подсвеченными совпадениями" },
      regex: { label: "Regex-пресет", title: "Regex-пресеты для повторяющихся шаблонов.", text: "Используйте регулярные выражения и встроенные пресеты для диагностики, email, IP, дат, TODO-маркеров и другого структурированного текста.", alt: "Regex-поиск FindText с диагностическим пресетом" },
      export: { label: "Экспорт результатов", title: "Экспортируйте результаты как Markdown, CSV, JSON или обычный текст.", text: "Откройте меню экспорта на панели результатов и выберите формат для отправки, архивации или дальнейшей обработки.", alt: "Меню экспорта FindText с форматами Markdown, CSV, JSON и обычный текст" },
      replace: { label: "Предпросмотр замены", title: "Предпросмотр замен перед любой записью.", text: "Проверяйте каждый кандидат по файлу и вхождению, выбирайте только нужные изменения и дайте FindText проверить исходный текст перед применением.", alt: "Предпросмотр замены FindText с выбираемыми изменениями по файлам и вхождениям" },
      history: { label: "История", title: "Снимки истории для завершенных расследований.", text: "Возвращайтесь к прежнему запросу с исходными настройками, числами результатов, путями файлов и подсвеченными совпадениями.", alt: "История поиска FindText с предыдущими локальными результатами" }
    },
    privacy: {
      eyebrow: "Приватность и безопасность",
      title: "Ваши файлы обрабатываются на вашем Mac.",
      text: "FindText спроектирован как локальный инструмент. Текущие функции не собирают данные, не отслеживают пользователей, не создают облачные индексы и не загружают содержимое файлов. Доступ к папкам выдается через sandbox macOS и хранится как security-scoped bookmarks.",
      cards: [
    ...
    faq: {
      ...
      items: [
        { question: "FindText загружает мои файлы?", answer: "Нет. Поиск выполняется локально на вашем Mac. Запросы, пути файлов и содержимое файлов не отправляются на сервер функциями, описанными здесь." },
        ...
        { question: "Можно ограничить поиск определенными типами файлов?", answer: "Да. Добавьте расширения вроде .md, .txt, .log, .swift или .docx. Если список пуст, FindText ищет читаемые текстовые файлы, которые не пропущены правилами." }
      ]
    },
